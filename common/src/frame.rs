...
pub enum FrameError {
    ...
    #[error(
        "session-list json_len {len} mismatches payload (payload {payload}, expected {expected})"
    )]
    SessionListJsonLen {
        len: u32,
        payload: u32,
...
impl Frame {
    ...
    pub fn ping(payload: Vec<u8>) -> Self {
        Self {
            ...
            body: Body::Ping(payload),
        }
    }
    pub fn pong(payload: Vec<u8>) -> Self {
        Self {
            stream_id: CONTROL_STREAM,
            body: Body::Pong(payload),
        }
    }
    pub fn hello(payload: Vec<u8>) -> Self {
        Self {
            stream_id: CONTROL_STREAM,
            body: Body::Hello(payload),
        }
    }
    pub fn paste_begin(
        stream_id: u32,
    ...
    pub fn kill_session_ack(request_id: u32, status: u8) -> Self {
        Self {
            stream_id: CONTROL_STREAM,
            body: Body::KillSessionAck { request_id, status },
        }
    }
        ...
        let payload: std::borrow::Cow<'_, [u8]> = match &self.body {
            ...
            Body::PasteBegin {
                ...
                let n = if name.len() <= max {
                    ...
                    let mut k = max;
                    while k > 0 && !name.is_char_boundary(k) {
                        k -= 1;
                    }
                    k
                };
            ...
            Body::ControllerChanged { status } => std::borrow::Cow::Owned(vec![*status]),
        ...
        match ty {
            FrameType::Data => {
                if stream_id == CONTROL_STREAM {
                    return Err(FrameError::DataOnControlStream);
                }
                if len > MAX_DATA_LEN {
            ...
            FrameType::Close => {
                ...
                if len != 0 {
                    return Err(FrameError::InvalidCloseLen(len));
                }
            }
    ...
    pub fn from_payload(
        ...
        let body = match ty {
            ...
            FrameType::SessionList => {
                ...
                let request_id =
                    u32::from_be_bytes([payload[0], payload[1], payload[2], payload[3]]);
                let json_len = u32::from_be_bytes([payload[4], payload[5], payload[6], payload[7]]);
...
mod tests {
    ...
    fn paste_begin_empty_name() {
        ...
        let enc = f.encode();
        let back =
            Frame::from_payload(2, FrameType::PasteBegin, enc[HEADER_LEN..].to_vec()).unwrap();
        match back.body {
    ...
    fn paste_chunk_zero_bytes_is_legal() {
        ...
        let back =
            Frame::from_payload(1, FrameType::PasteChunk, enc[HEADER_LEN..].to_vec()).unwrap();
        match back.body {
    ...
    fn paste_reject_all_reasons_decode() {
        for r in 0..=PASTE_REJECT_MAX_REASON {
            ...
            let enc = f.encode();
            let back =
                Frame::from_payload(1, FrameType::PasteReject, enc[HEADER_LEN..].to_vec()).unwrap();
            match back.body {
    ...
    fn download_chunk_round_trip() {
        ...
        let enc = f.encode();
        let back =
            Frame::from_payload(2, FrameType::DownloadChunk, enc[HEADER_LEN..].to_vec()).unwrap();
        match back.body {
            Body::DownloadChunk {
                download_id,
                bytes: b,
    ...
    fn download_chunk_rejects_oversize() {
        let e = Frame::validate_header(
            1,
            FrameType::DownloadChunk as u8,
            MAX_DOWNLOAD_CHUNK_LEN + 1,
        )
        .unwrap_err();
        assert!(matches!(e, FrameError::PayloadTooLarge { .. }));
    }
    ...
    fn download_end_round_trip_ok_and_cancel() {
        for st in [DOWNLOAD_STATUS_OK, DOWNLOAD_STATUS_CANCEL] {
            ...
            let enc = f.encode();
            let back =
                Frame::from_payload(2, FrameType::DownloadEnd, enc[HEADER_LEN..].to_vec()).unwrap();
            match back.body {
                Body::DownloadEnd {
                    download_id,
                    status,
    ...
    fn session_list_round_trip() {
        let json =
            br#"{"sessions":[{"id":"abc","idle_secs":12,"attached":2,"has_controller":true}]}"#;
        let f = Frame::session_list(42, json.to_vec());
        let enc = f.encode();
        let (ty, _) = Frame::validate_header(
            0,
            enc[4],
            u32::from_be_bytes([enc[5], enc[6], enc[7], enc[8]]),
        )
        .unwrap();
        let back = Frame::from_payload(0, ty, enc[HEADER_LEN..].to_vec()).unwrap();
        match back.body {
    ...
    fn kill_session_ack_round_trip() {
        for st in [KILL_STATUS_OK, KILL_STATUS_NOT_FOUND] {
            ...
            let enc = f.encode();
            let back =
                Frame::from_payload(0, FrameType::KillSessionAck, enc[HEADER_LEN..].to_vec())
                    .unwrap();
            match back.body {
                Body::KillSessionAck { request_id, status } => {
