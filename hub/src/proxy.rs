...
//! gated by the `ListenerMode` extension. On the no-auth listener the
//! token requirement is dropped; the origin check is always enforced.
//!
...
pub async fn term_ws(
    ...
    Extension(mode): Extension<ListenerMode>,
    headers: HeaderMap,
) -> axum::response::Response {
    // Origin check. Always enforced — required even on the no-auth
    // listener because WebSockets are not subject to CORS and any web
    // origin that can reach the perimeter could otherwise open a
    // terminal WS.
    if check_origin(&headers, &mode.origin).is_err() {
        return (StatusCode::FORBIDDEN, "bad origin").into_response();
    }

    // Bearer via subprotocol — REQUIRED on the authed listener,
    // OPTIONAL on the no-auth listener. We still echo the selected
    // subprotocol back when the browser sent one so that legacy SPAs
    // (and the same SPA in mixed-mode deployments) continue to work.
    let chosen_subprotocol = if mode.no_auth {
        token_from_ws_protocol(&headers).map(|(_tok, sp)| sp)
    } else {
